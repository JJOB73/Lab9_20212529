...
<div class='container'>
    ...
    <div class="row mb-5 mt-4">
        ...
        <div class="col-lg-6 my-auto text-lg-right">
            <a href="<%= request.getContextPath()%>/ArbitroServlet?action=irCrear" class="btn btn-primary">Crear Árbitro</a>
        </div>
    ...
    <table class="table">
        ...
        <%
            List<Arbitro> lista = (List<Arbitro>) request.getAttribute("listaArbitros");
            int index = 1;
            for (Arbitro ar : lista) {
        %>
        <tr>
            <td><%=index%></td>
            <td><%=ar.getNombre()%></td>
            <td><%=ar.getPais()%></td>
            <td>
                <a href="<%= request.getContextPath() %>/ArbitroServlet?action=borrar&id=<%= ar.getIdArbitro() %>">Borrar</a>
            </td>
        </tr>
        <%
                index=index+1;
